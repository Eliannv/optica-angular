...
export class AuthService {
   ...
   * @param rememberMe Si es true, mantiene la sesión activa (persistencia local)
   */
  login(email: string, password: string, rememberMe: boolean = false): Observable<Usuario> {
    ...
    // Configurar persistencia ANTES de hacer login
   ...
   * Espera a que Firebase Auth cargue el estado de persistencia y verifica si hay un usuario válido
      ...
      switchMap(firebaseUser => {
        ...
        return this.getUserData(firebaseUser.uid).pipe(
          map(userData => {
            ...
            // En sesión restaurada, NO validar Machine ID/Sucursal
